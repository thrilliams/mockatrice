...
export const Card = z
	.object({
		...
		card_back_id: z.string().uuid().optional(),
		...
		full_art: z.boolean(),
		games: z.union([z.literal('paper'), z.literal('arena'), z.literal('mtgo')]).array(),
		...
		preview: z
			.object({
				previewed_at: z.string().transform((string) => new Date(string)),
				source_uri: z.string(),
				source: z.string()
			})
			.strict()
